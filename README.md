...
当前版本：`0.4.0`。已在官方 standalone `@deepseek-ai/dsh@0.1.5-rc.2` 的真实 Web 宿主中验证加载和主要设置行为。真实 Electron 桌面壳、`dsh-web-all` 与第三方皮肤管理器尚未实测；不能将 Web 验证等同于这些环境已兼容。
...
- 五项偏好、保存草稿、多窗口冲突处理及专注模式沿用原行为；会话背景仍为已保存强度的40%，不擅自覆盖用户设置。

0.1.x–0.3.x升级时，停止对应profile，添加下方 **0.4.0** 安装包后重启并刷新客户端。旧版原图保留在源码素材目录，当前运行产物只使用本轮精绘人物、穹顶背景和既有缎带。
...
```text
dist/dsh-skin-asuka-p01-0.4.0.tgz
```

先停止准备安装的 Harness profile，然后执行：

```sh
dsh plugin --profile web add "/绝对路径/dsh-skin-asuka-p01-0.4.0.tgz"
dsh web --no-open
```
...
`npm pack` 会自动执行 `prepack`，依次进行 TypeScript 检查与构建，生成 `dist/dsh-skin-asuka-p01-0.4.0.tgz`。如果只需要查看构建产物而暂不打包，可单独执行：
