...
当前版本：`0.1.0`。已在官方 standalone `@deepseek-ai/dsh@0.1.5-rc.2` 的真实 Web 宿主中验证加载和主要设置行为。真实 Electron 桌面壳、`dsh-web-all` 与第三方皮肤管理器尚未实测；不能将 Web 验证等同于这些环境已兼容。
...
```text
dist/dsh-skin-asuka-p01-0.1.0.tgz
```

先停止准备安装的 Harness profile，然后执行：

```sh
dsh plugin --profile web add "/绝对路径/dsh-skin-asuka-p01-0.1.0.tgz"
dsh web --no-open
```
...
| 人物位置 | 左侧留白 / 右侧留白 | 左侧留白 | 仅在所选侧有足够空间时显示人物 |
| 背景强度 | 0–100% | 55% | 调整公园背景透明度，不修改正文透明度 |
...
`npm pack` 会自动执行 `prepack`，依次进行 TypeScript 检查与构建，生成 `dist/dsh-skin-asuka-p01-0.1.0.tgz`。如果只需要查看构建产物而暂不打包，可单独执行：
