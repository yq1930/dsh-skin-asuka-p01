...
`dsh-skin-asuka-p01` 是 DeepSeek Harness 的独立皮肤组合包，采用 P01 双人物立绘、酒红金饰框架、独立回复卡片及公园日夜背景。面向桌面尺寸的 Harness Web 渲染界面，保留宿主会话、模型、工具与审批行为。

当前版本：`0.3.0`。已在官方 standalone `@deepseek-ai/dsh@0.1.5-rc.2` 的真实 Web 宿主中验证加载和主要设置行为。真实 Electron 桌面壳、`dsh-web-all` 与第三方皮肤管理器尚未实测；不能将 Web 验证等同于这些环境已兼容。

## 0.3.0 双立绘与金饰框架

本轮保留P01经典赛璐璐的五官和画风，为两侧分别制作完整人物形象，并将装饰从细边框扩展为酒红丝绒、香槟金卷草与宝石缎带。没有使用参考主题的女仆或蕾丝图片。

- **左侧迎接形象**：P01原日常装、开掌迎接；**右侧阅读形象**：红色外套、深色百褶裙、抱笔记本。欢迎页展示两张不同全身立绘，脚底对齐，不镜像复制，不再使用右上头像代替人物。
- 会话中主角色换用原P01收手站姿，避免展开手势把角色压得过小；两边都有余白时统一人物高度，并分别避让正文、代码和输入区域。窄窗可独立隐藏角色。
- 完整金饰舞台框架：顶部酒红横饰、四角卷草、重复金边、中央徽章和底部收尾纹饰。
- 侧栏增加丝绒质感、品牌铭牌、奶油金边新会话按钮、工作区绸带条与肖像徽框。折叠后回到简洁图标栏。
- 输入框使用金属双边、细密纹饰和独立透明宝石缎带。欢迎页与会话底部为装饰预留空间，不覆盖文字和操作按钮。
- 保留独立实色回复卡片、24px上下阅读留白和随列宽变化的32/24/16px左右留白；表格、代码仍在卡片内横向滚动。
- 会话背景继续使用用户设置强度的40%，欢迎页使用原值。专注模式隐藏人物与场景；现有五项偏好不需迁移。
- 修复重新打开设置页后快速切换选项可能被初始化覆盖的问题；保存失败、多窗口冲突仍保留草稿。

已有0.1.x/0.2.x版本时，停止对应profile后添加下面的 **0.3.0** 安装包，再重启并刷新客户端。人物位置设置用于交换主角色和阅读角色所在侧。
...
```text
dist/dsh-skin-asuka-p01-0.3.0.tgz
```

先停止准备安装的 Harness profile，然后执行：

```sh
dsh plugin --profile web add "/绝对路径/dsh-skin-asuka-p01-0.3.0.tgz"
dsh web --no-open
```
...
| 设置 | 选项或范围 | 默认值 | 行为 |
| --- | --- | --- | --- |
| 表现方式 | 角色模式 / 专注模式 | 角色模式 | 专注模式隐藏人物、公园背景和角色头像，保留配色与 `02` |
| 显示公园背景 | 开 / 关 | 开 | 不影响人物和主题配色；专注模式下不可调整 |
| 主立绘位置 | 左侧 / 右侧 | 左侧 | 阅读立绘位于另一侧；会话中按各侧空间显示 |
| 背景强度 | 0–100% | 55% | 欢迎页采用此值；会话采用其40%，正文始终实色 |
| 人物大小 | 60–120% | 100% | 是期望比例，仍受窗口高度与实际留白限制 |

修改表单只改变**当前设置页的草稿预览**。点击“保存”后，客户端核对 Host 返回的有效值，再将其应用到工作区。

- **恢复默认**：先恢复默认草稿，再点击保存；不重置宿主明暗模式或正文字号。
...
明暗外观继续在宿主 **设置 → 通用 → 外观** 中调整。字号、模型和审批规则继续由宿主管理。
...
`npm pack` 会自动执行 `prepack`，依次进行 TypeScript 检查与构建，生成 `dist/dsh-skin-asuka-p01-0.3.0.tgz`。如果只需要查看构建产物而暂不打包，可单独执行：
...
构建流程将 `assets/source/` 中的人物和公园素材编码为 WebP，并内嵌至客户端产物。构建保留源素材，输出 `lib/index.js` 和 `lib/client.js`；浏览器产物使用 Harness module-loader factory 格式，React、Cordis 和宿主 UI 共享模块保持 external，不打入第二份运行时。
...
assets/source/            P01原图、迎接/阅读立绘、宝石缎带及公园背景
