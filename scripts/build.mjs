...
const [front, welcome, study, portrait, day, night, ribbon] = await Promise.all([
  encode('asuka-front.png', 520, true),
  encode('asuka-welcome.png', 640, true),
  encode('asuka-study.png', 580, true),
...
console.log(`Built ${pkg.name}@${pkg.version}; client ${(Buffer.byteLength(client) / 1024).toFixed(0)} KiB; artwork ${[front, welcome, study, portrait, day, night, ribbon].reduce((n, a) => n + a.bytes, 0)} bytes.`)
