  ...
  return function AsukaSettings({ close }: { close: () => void }) {
    ...
    return <section className="asuka-settings" aria-label="明日香 P01 主题设置">
      ...
      <div>
        ...
        <div className="asuka-settings__preview" style={previewStyle} data-preview-phase={previewPhase} data-preview-side={draft.side} data-preview-mode={draft.presentation} aria-label="当前草稿预览">
          ...
          {draft.presentation === 'character' ? <>
            <img src={previewPhase === 'hero' ? artwork.welcome : artwork.front} alt="明日香白衣立绘" style={{ objectFit: 'contain', height: `${Math.min(94, (previewPhase === 'hero' ? 80 : 50) * draft.artScale / 100)}%`, width: '46%', position: 'absolute', bottom: '4%', [draft.side]: 0 }} />
            <img src={artwork.study} alt="明日香绯红立绘" style={{ objectFit: 'contain', height: `${Math.min(92, (previewPhase === 'hero' ? 78 : 50) * draft.artScale / 100)}%`, width: '44%', position: 'absolute', bottom: '4%', [draft.side === 'left' ? 'right' : 'left']: 0 }} />
      ...
      <div className="asuka-settings__controls">
        ...
        <label className="asuka-settings__field">背景强度 <output>{draft.intensity}%</output><input type="range" min="0" max="100" step="1" value={draft.intensity} disabled={!writable || saving || !draft.background || draft.presentation === 'focus'} onChange={e => change('intensity', Number(e.target.value))} /><small>欢迎页使用此强度；会话中两侧为其40%，中间进一步淡化，正文保持实色底。</small></label>
