  ...
  return function AsukaSettings({ close }: { close: () => void }) {
    ...
    return <section className="asuka-settings" aria-label="明日香 P01 主题设置">
      ...
      <div>
        ...
        <div className="asuka-settings__preview" style={previewStyle} data-preview-phase={previewPhase} data-preview-side={draft.side} data-preview-mode={draft.presentation} aria-label="当前草稿预览">
          ...
          {draft.presentation === 'character' ? <>
            <img src={previewPhase === 'hero' ? artwork.welcome : artwork.front} alt="明日香迎接立绘" style={{ objectFit: 'contain', height: `${Math.min(98, 90 * draft.artScale / 100)}%`, width: '46%', position: 'absolute', bottom: 0, [draft.side]: 0 }} />
            <img src={artwork.study} alt="明日香阅读立绘" style={{ objectFit: 'contain', height: `${Math.min(96, 86 * draft.artScale / 100)}%`, width: '44%', position: 'absolute', bottom: 0, [draft.side === 'left' ? 'right' : 'left']: 0 }} />
        ...
        <p className="asuka-settings__notice">欢迎页展示迎接与阅读双立绘；会话中主角色切换为收手站姿。此处为草稿示意，保存后应用。</p>
      </div>
      <div className="asuka-settings__controls">
        ...
        <label className="asuka-settings__field">主立绘位置<select value={draft.side} disabled={!writable || saving || draft.presentation === 'focus'} onChange={e => change('side', e.target.value as AsukaPreferences['side'])}><option value="left">左侧，阅读立绘在右</option><option value="right">右侧，阅读立绘在左</option></select></label>
